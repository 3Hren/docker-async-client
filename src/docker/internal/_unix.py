...
class _UnixHTTPConnection(_HTTPConnection):
    def __init__(self, prefix, io_loop, client, request, release_callback, final_callback, max_buffer_size):
        ...
        request.url = 'http://localhost{0}'.format(request.url[request.url.index(prefix) + len(prefix):])

        class NoneResolver(object):
            def resolve(self, host, port, family, callback):
                io_loop.add_callback(callback, ((socket.AF_UNIX, path),))
        super(_UnixHTTPConnection, self)\
            .__init__(io_loop, client, request, release_callback, final_callback, max_buffer_size, NoneResolver())
